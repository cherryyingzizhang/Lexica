...
public class ItemDetailFragment extends Fragment{
	...
	int dp200;
	Database db;
	TextView word, meaning, expandable;
...
//					+ ":" + String.format(Locale.US,"%02d", milliseconds)
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		meaning = (TextView) container.findViewById(R.id.tvMeaning);
		...
		texttoSpeechItem.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View arg0) {
				...
				{
					ItemListActivity.speakOut(getActivity(), mItem.item);
				}
				else //fragment launched from ItemDetailActivity
				{
					ItemDetailActivity.speakOut(getActivity(), mItem.item);
				}
			}});
		...
		texttoSpeechDesc.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View arg0) {
				if (ItemListActivity.mTwoPane) //fragment launched two-pane from ItemListActivity
				{
					ItemListActivity.speakOut(getActivity(), mItem.description);
				}
				else //fragment launched from ItemDetailActivity
				{
					ItemDetailActivity.speakOut(getActivity(), mItem.description);
				}
			}});
	...
	private void changeFont()
	{
		...
		{
			word.setTypeface(Typeface.DEFAULT);
			meaning.setTypeface(Typeface.DEFAULT);
		}
		else if (font.contentEquals("Handwriting"))
		{
			word.setTypeface(Typeface.createFromAsset(getActivity().getAssets(), "fonts/handwriting.ttf"));
			meaning.setTypeface(Typeface.createFromAsset(getActivity().getAssets(), "fonts/handwriting.ttf"));
		}
		else if (font.contentEquals("3d"))
		{
			word.setTypeface(Typeface.createFromAsset(getActivity().getAssets(), "fonts/3d.ttf"));
			meaning.setTypeface(Typeface.createFromAsset(getActivity().getAssets(), "fonts/3d.ttf"));
		}
	}
